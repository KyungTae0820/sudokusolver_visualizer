        ...
        if flag == 0:
            ...
            for y in range(9):
                ...
                for x in range(9):
                    ...
                    cv2.imwrite("mat" + str(y) + str(x) + ".png", img)
                    x_input = img.reshape(1, 28, 28, 1).astype("float32") / 255.0  # normalize for safety
                    if x_input.sum() > 1.0:  # changed threshold to match normalization
                        prediction = classifier.predict(x_input)
                        digit = int(np.argmax(prediction))
                        ligne += "{:d}".format(digit)
                    else:
                        ligne += "0"
